...
// port2 for swapping to the other classification mode
const port2 = parseInt(process.env.PORT, 10) || 8081
...
var unhook_intercept = intercept(function(text) {
  var stdoutText = text.toString();
    ...
    var fs = require('fs');
  var outString = JSON.stringify(stdoutText);
    // fs.writeFile("/home/wysetime/CAT400/opendatacam/error-console.text", JSON.stringify(outString, null, 4), err => {
    //     if (err) throw err;
    // });
    fs.appendFile('/home/wysetime/CAT400/opendatacam/error-console.text', JSON.stringify(outString, null, 4), function (err) {
        if (err) throw err;
    });
  ...
  if(stdoutText.indexOf('Video stream:') > -1) {
    ...
    console.log('videoResolution1 = '+videoResolution.w);
    console.log('videoResolution1 = '+videoResolution.h);
